...
foreach ($uniqbits as $k=>$v) {
	...
	$BITTOTZ[]=array("bit"=>$k,"total"=>7979);
...
foreach ($BITEPZ['episodes'] as $ep) {
	foreach($ep['bits'] as $k => $v){
		...
		$tbit = array_filter($BITTOTZ,function($b) use ($k){
			// echo "bbit:".$b['bit'];
			// echo "k:".$k;
			return $b["bit"]==$k;
		});
		...
		$dump.="<li>".$v." <em><a href='#query/episode:".$ep['episode']." AND bit:%22".urlencode($k)."%22'>".$k."</a></em> <a href='#query/bit:%22".urlencode($k)."%22'>".$BITTOT." total</a>)</li>";
