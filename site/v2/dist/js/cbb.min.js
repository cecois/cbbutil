...
"use strict";var QueryView=Backbone.View.extend({el:$("#query-form"),template:CBB.templates.queryViewTpl,events:{"click #cbb-bt-search":"set","click #cbb-bt-random":"rando"},bindings:{input:"raw"},initialize:function(){return this.render()},rando:function(){var t=String.fromCharCode(Math.floor(26*Math.random())+97)+"*",e=String.fromCharCode(Math.floor(26*Math.random())+97)+"*";return appQuery.set({raw:t+e}),this},set:function(t){return this.model.set({raw:"episode:200"}),this},render:function(){return $(this.el).html(this.template(this.model.toJSON())),this.stickit()}});
...
"use strict";var Locations=Backbone.Collection.extend({model:Bit,initialize:function(t){t||(t={})},url:function(){var t=null;switch(CONFIG.mode){case"T":case"33":t="http://localhost:3030/geoms/cbb?q="+appState.get("locations");break;default:t="http://milleria.org:3030/geoms/cbb?q="+appState.get("locations")}return t},sync:function(t,a,c){return c.dataType="jsonp",c.jsonpCallback="cwmccallback",Backbone.sync(t,a,c)}});
"use strict";var LocationsView=Backbone.View.extend({el:null,events:{},initialize:function(){(new L.featureGroup).addTo(map);return this.collection.bind("sync",this.render,this),this},render:function(){return BitGroup.clearLayers(),this.collection.each(function(e,t){var o=(0,CBB.templates.bitMarkerViewTpl)(e.get("properties")),i=e.get("geometry").type,r=this.get_style(),n=this.get_style("active"),c=this.get_style("seen"),l=e.attributes;if("point"==i.toLowerCase()){(s=L.geoJson(l,{seen:!1,location_id:e.get("properties").cartodb_id,location_type:e.get("geometry").type.toLowerCase(),pointToLayer:function(e,t){return L.circleMarker(t,r)}})).bindPopup(o).addTo(BitGroup).on("click",function(e){e.target.setStyle(n)}).addOneTimeEventListener("popupopen",function(e){})}else{var s=L.geoJson(l,{seen:!1,location_id:e.get("properties").cartodb_id,location_type:e.get("geometry").type.toLowerCase(),style:r});s.bindPopup(o).addTo(BitGroup).on("click",function(e){e.target.setStyle(n);_.find(BitGroup._layers,function(e){return 1==e.options.seen})}).addOneTimeEventListener("popupopen",function(e){})}s.on("popupclose",function(e){e.target.setStyle(c)}),void 0===l.options&&(l.options={cartodb_id:null}),l.options.cartodb_id=e.get("properties").cartodb_id.toString()},this),this.collection.length>0&&map.fitBounds(BitGroup.getBounds()),appActivityView.stfu(),this},get_style:function(e){switch(e){case"active":return{radius:18,fillColor:"#fecd0b",color:"#000",weight:1,opacity:1,fillOpacity:.6};case"seen":return{radius:6,fillColor:"#ffffff",color:"#1288b9",weight:1,opacity:.6,fillOpacity:.3};case"linenew":return{fillColor:"rgba(126, 223, 216, 100)",color:"rgba(126, 223, 216, 100)",weight:6,opacity:.8};case"lineactive":return{fillColor:"#fecd0b",color:"#fecd0b",weight:8,opacity:1};case"lineseen":return{fillColor:"#ffffff",color:"#ffffff",weight:6,opacity:.6};default:return{radius:6,fillColor:"rgba(6, 6, 6, 50)",color:"#000",weight:1,opacity:1}}}});
...
"use strict";var Bits=Backbone.Collection.extend({model:Bit,url:function(){appQuery.get("raw");var e=null;switch(CONFIG.mode){case"T":default:e=CONFIG.index_root}return e},initialize:function(e){return e||(e={}),this.listenTo(appQuery,"change:raw",this.upf),this.listenTo(appQueryFacets,"add remove",this.upf),this},upf:function(){return this.fetch()},sync:function(e,t,s){return s.dataType="json",s.async=!0,s.url=this.url(),s.method="POST",s.data=JSON.stringify(appQuery.queryobj()),Backbone.sync(e,t,s)},parse:function(e){appActivity.set({message:"extracting locations..."});var t=_.map(_.filter(e.hits.hits,function(e){return"Location"==e._source.bit}),function(e){return e._source.location_type+":"+e._source.location_id});appState.set({locations:t.join(",")}),appLocations.length>0&&appLocations.fetch(),appActivity.set({message:"setting facets..."});var s=_.map(e.aggregations.all_bits.bits.filtered_bits.buckets,function(e,t){var s={bit:e.key},a=appQueryFacets.findWhere(s)?"is-active":"";return{type:"bits",facet:e.key,count:e.doc_count,active:a}});if(appFacetsBits.reset(s),void 0!==e.aggregations.all_bits.tags){var a=_.map(e.aggregations.all_bits.tags.filtered_tags.buckets,function(e,t){var s={tags:e.key},a=appQueryFacets.findWhere(s)?"is-active":"";return{type:"tags",facet:e.key,count:e.doc_count,active:a}});appFacetsTags.reset(a)}if(void 0!==e.aggregations.all_bits.guests){var i=_.map(e.aggregations.all_bits.guests.filtered_guests.buckets,function(e,t){var s={episode_guests:e.key},a=appQueryFacets.findWhere(s)?"is-active":"";return{type:"guests",facet:e.key,count:e.doc_count,active:a}});appFacetsGuests.reset(i)}if(void 0!==e.aggregations.all_bits.episodes){var r=_.map(e.aggregations.all_bits.episodes.filtered_episodes.buckets,function(e,t){var s={episode:e.key},a=appQueryFacets.findWhere(s)?"is-active":"";return{type:"episode",facet:e.key,count:e.doc_count,active:a}});appFacetsEpisodes.reset(r)}appState.set({search_results_count:e.hits.hits.length}),appActivity.set({message:"done!",hang:10});return _.map(e.hits.hits,function(e){var t=e._source,s=e._source.created_at==e._source.updated_at?null:moment(e._source.updated_at).format("YYYY.MMM.DD"),a=null==e._source.episode_title||""==e._source.episode_title?e._source.episode.split("/")[e._source.episode.split("/").length-1]:e._source.episode_title;return t.meta={created:moment(e._source.created_at).format("YYYY.MMM.DD"),updated:s,episode_string:a,tags:e._source.tags.split(",")},t})}});
...
"use strict";var State=Backbone.Model.extend({defaults:{locations:null},initialize:function(t){return t||(t={}),this},slugify:function(t){var e=this.get("slugs"),s=_.map(e,function(e){return e.slug==t?{name:e.name,slug:e.slug,active:"is-active"}:{name:e.name,slug:e.slug,active:null}});return this.set({slugs:s}),this}});
"use strict";var StateViewDownMenu=Backbone.View.extend({el:$("#cbb-bt-map-toggle"),template:CBB.templates.statesViewTpl_default,events:{"click i":"setm"},initialize:function(){return this.model.bind("change:downout",this.render,this),this.render()},setm:function(){console.log("setm...");var t="out"==this.model.get("downout")?"down":"out";return this.model.set({downout:t}),this},render:function(){return"down"==this.model.get("downout")?$("#cbb-main").addClass("down"):$("#cbb-main").removeClass("down"),$(this.el).html(this.template(this.model.toJSON())),this},rewire:function(){return 1==CONFIG.verbose&&console.info("------\x3e StatesVIew --\x3e rewire"),$(this.el).find('[data-toggle="tooltip"]').tooltip("destroy"),$(this.el).find('[data-toggle="tooltip"]').tooltip({position:"right",container:"body"}),this},reset:function(){return 1==CONFIG.verbose&&console.info("------\x3e StatesVIew --\x3e reset"),this.render()}});
"use strict";var SlugsViewPanes=Backbone.View.extend({el:$("#cbb-main-panes"),events:{"click .cbb-trigger":"trigger"},initialize:function(){return this.collection.bind("change",this.render,this),this.render()},trigger:function(t){if(t.preventDefault(),""==$(t.currentTarget).attr("data-type"))e=$(t.currentTarget).attr("data-target");else var e=$(t.currentTarget).attr("data-type")+':"'+$(t.currentTarget).attr("data-target")+'"';return appQuery.set({raw:e}),appSlugs.switch("search"),this},setp:function(t){return t.preventDefault(),appState.slugify($(t.currentTarget).attr("data-id")),this},render:function(){var t=this.collection.active().get("slug"),e=null;e=CBB.templates["0-"+t],$(".cbb-main-pane").each(function(){0==$(this).hasClass("hidden")&&$(this).addClass("hidden")});var a=$(".cbb-main-pane[id='pane-"+t+"']");return"search"!==t&&"browse"!==t&&"update"!==t&&$(a).html(e(this.collection.toJSON())),$(a).hasClass("hidden")&&$(a).removeClass("hidden"),this}});
...
"use strict";var UpdatesView=Backbone.View.extend({el:$("#pane-update"),template:CBB.templates.updateViewTpl,events:{},initialize:function(){return this.collection.bind("sync",this.render,this),this.collection.fetch()},render:function(){var e=[],t=this.collection.models[0].get("report");for(this.collection.models[0].get("date"),this.collection.models[0].get("episodes"),this.collection.models[0].get("anno");t.length>0;)e.push(t.splice(0,4));return $(this.el).html(this.template()),this}});
...
"use strict";var BitsView=Backbone.View.extend({el:"#search-bits",template:CBB.templates.bitsView,events:{"click i.fa-map-marker":"zoomto","click .cbb-trigger":"trigger","click .cbb-bit-meta-bt":"meta"},initialize:function(){return this.collection.bind("sync",this.render,this),this.proxyfetch()},meta:function(t){t.preventDefault();var e=$(t.currentTarget).parents(".bit-data").find(".bit-data-meta");return $(t.currentTarget).toggleClass("active"),e.toggleClass("active"),this},trigger:function(t){t.preventDefault();var e=$(t.currentTarget).attr("data-type")+':"'+$(t.currentTarget).attr("data-target")+'"';return appQuery.set({raw:e}),this},zoomto:function(t){t.preventDefault();var e=$(t.currentTarget).attr("data-id").split(":"),r=_.find(BitGroup.getLayers(),function(t){return t.options.location_id==e[1]&&t.options.location_type.indexOf(e[0])>=0});return map.fitBounds(r.getBounds()),this},proxyfetch:function(){return this.collection.fetch(),this},render:function(){if(this.collection.length<1){var t=null==appQuery.get("raw")||""==appQuery.get("raw")?"null query":appQuery.get("raw")+appQuery.query_facetadd();$(this.el).html("no bits for "+t)}else $(this.el).html(this.template({rows:this.collection.toJSON()}));return appActivityView.stfu(),this}});
